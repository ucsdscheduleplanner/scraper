...
class TimeIntervalCollection:
    ...
    def get_days(day_list):
        ...
        If we are given an input with F before MW, we return them in
        the same format
        ...
        ret_days = [x for x in ret_days if x is not None and x != " "]
        return ret_days
        ...
        for t_interval in t_intervals:
            ...
            for hour_time in t_interval.split('-'):
                ...
                try:
                    ...
                    formatted_interval = datetime.strptime("12:00pm", '%I:%M%p')
                    logging.warning("Failed to process section {} {}".format(section.department, section.course_num))
